      ...
      <div className="gap-6 relative">
        <div className="z-10 relative max-w-desktop w-[90%] md:w-3/4 lg:w-1/2 mx-auto flex items-center text-center justify-center md:px-10 pt-10 pb-16 lg:pb-24 md:pt-16">
          <div className="bg-white shadow-2xl py-10 px-6">
            ...
            <p className="text-2xl pb-4">{t('welcome1')}</p>
            <p className="text-2xl pb-4">{t('welcome2')}</p>
            <p className="px-4 md:px-10 pb-6 font-bold text-center text-2xl">{t('partyDate')}</p>
            <p className="text-4xl font-great-vibes">{t('location')}</p>
            <a href="https://maps.app.goo.gl/GCNGLwmFxMhSHnGCA" className="underline">
              {t('address')}
            </a>
            <p className="text-2xl pt-6">{t('arrivingTime')}</p>
            <p>15:30 - 16:30</p>
          </div>
        </div>
      </div>
